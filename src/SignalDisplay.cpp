...
/*void SignalDisplay::setNewValues(std::vector<float> aFCurvesValues)
{
	...
	else
	{
		...
		{
			if (m_bVerbose)
                std::cerr << "[ERROR] (SignalDisplay::setNewValues) the size of the vector doesn't match the predefined number of curves! " << std::endl;
			m_bDrawCurves = false;
			return;
		}
		else
...
void SignalDisplay::paintEvent(QPaintEvent *)
{
